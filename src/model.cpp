...
void Model::Init(const char*modelPath) {
	...
	int nFileSize = 0;
	std::shared_ptr<unsigned char> fileContent = LoadFileContent(modelPath, nFileSize);
	if (!fileContent){
		return;
	}
	std::vector<FloatData> positions, texcoords, normals;
	std::vector<VertexDefine> vertexes;
	std::stringstream ssFileContent((char*)fileContent.get());
	std::string temp;
	char szOneLine[256];
	...
	mShader.Init("../resources/model.vs", "../resources/model.fs");
...
void Model::Draw(float cameraX, float cameraY, float cameraZ, glm::mat4 &viewMatrix, glm::mat4 &projectionMatrix) {
	...
	mShader.SetVec4("U_cameraPos", cameraX, cameraY, cameraZ, 1.0f);
	glm::mat4 it_modelMatrix = glm::inverseTranspose(mModelMatrix);
	mVertexBuffer.Bind();
	mShader.Bind(glm::value_ptr(mModelMatrix), glm::value_ptr(viewMatrix), glm::value_ptr(projectionMatrix));
	glUniformMatrix4fv(mShader.GetIT_ModelMatrixLocation(), 1, GL_FALSE, glm::value_ptr(it_modelMatrix));
	glDrawArrays(GL_TRIANGLES, 0, mVertexBuffer.GetVertexCount());
